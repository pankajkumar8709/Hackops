from logging.config import fileConfig
import os
from sqlalchemy import create_engine, pool
from alembic import context

# Load .env so DATABASE_SYNC_URL is available
from dotenv import load_dotenv
load_dotenv(os.path.join(os.path.dirname(__file__), '..', '..', '.env'))

# Alembic Config object (used for logging setup only)
config = context.config

if config.config_file_name is not None:
    fileConfig(config.config_file_name)

# Import all models so autogenerate detects every table
from app.database import Base  # noqa: F401
import app.models  # noqa: F401 — registers all mappers on Base.metadata

target_metadata = Base.metadata

# Read URL directly from env — bypasses configparser interpolation
# (avoids ValueError when password contains URL-encoded chars like %40)
DB_SYNC_URL = os.environ["DATABASE_SYNC_URL"]


def run_migrations_offline() -> None:
    context.configure(
        url=DB_SYNC_URL,
        target_metadata=target_metadata,
        literal_binds=True,
        dialect_opts={"paramstyle": "named"},
    )
    with context.begin_transaction():
        context.run_migrations()


def run_migrations_online() -> None:
    connectable = create_engine(DB_SYNC_URL, poolclass=pool.NullPool)
    with connectable.connect() as connection:
        context.configure(
            connection=connection,
            target_metadata=target_metadata,
        )
        with context.begin_transaction():
            context.run_migrations()


if context.is_offline_mode():
    run_migrations_offline()
else:
    run_migrations_online()
